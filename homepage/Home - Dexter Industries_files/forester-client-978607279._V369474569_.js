...
var ue=CSM.ue,VERSION=CSM.ue_log_idx?"2":"1",FORESTER_URL="//"+ue.furl+"/1/batch/"+VERSION+"/OP/",GROUPS_SEP="/",GROUP_SEP="$",NAMESPACES_SEP="&",NAMESPACE_SEP="=",DATAS_SEP=",",PART_SEP="_",SEP=":",PERCENT_CHAR="%",MAX_WYW_SIZE=2000,PUBLISH_PERIOD=60000,MIN_DATA_SIZE=5,MAX_NAMESPACE_LOG_COUNT=1000,currentWywSize,groups,calls=0,sends=0,logCount={},size=function(stringOrNumber){return(""+stringOrNumber).length;
...
},log=function(data,namespace,options){logWithTime(data,namespace,options,ue.d());
...
if(logCount[logCountKey]==(MAX_NAMESPACE_LOG_COUNT+1)){log({m:message,f:"forester-client.js"},"jserr",options);
}return;
...
}else{if(size(encData)<=availableSizeForDataInEmptyWyw){send();
continue;
...
}if(currentAvailableSize==0||now){send();
}encData=restData;
}calls++;
...
},sendViaGet=function(isExecutedAtOnBeforeUnload){var finalUrlStr=buildUrlToForester();
if(!finalUrlStr){return;
}new Image().src=finalUrlStr;
sends++;
init();
},sendViaAjax=function(isExecutedAtOnBeforeUnload){var finalUrlStr=buildUrlToForester();
if(!finalUrlStr){return;
}if(isExecutedAtOnBeforeUnload||!CSM.ue_swi){new Image().src=finalUrlStr;
}else{if(xmlhttp&&browserSupportForCORS){requestsQueue.push(finalUrlStr);
if(requestsQueue.length==1){xmlhttp.open("GET",finalUrlStr,true);
xmlhttp.send();
}}else{new Image().src=finalUrlStr;
}}sends++;
init();
},send=sendViaGet,requestsQueue=[],prevOnBeforeUnload=w.onbeforeunload,browserSupportForCORS=0,xmlhttp;
if(CSM.ue_swi){if(CSM.ue_swm==4){FORESTER_URL="//"+ue.furl+"/1/batch/"+VERSION+"/OE/";
}if(CSM.ue_swm&&CSM.ue_swm!=1){send=sendViaAjax;
if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();
browserSupportForCORS=("withCredentials" in xmlhttp)?1:0;
}else{if(window.ActiveXObject&&window.XDomainRequest){var xhrVersions=["MSXML2.XMLHTTP.6.0","MSXML2.XMLHTTP.5.0","MSXML2.XMLHTTP.4.0","MSXML2.XMLHTTP.3.0","MSXML2.XMLHTTP","Microsoft.XMLHTTP"];
for(var i=0;
i<xhrVersions.length;
i++){try{xmlhttp=new ActiveXObject(xhrVersions[i]);
browserSupportForCORS=1;
}catch(e){continue;
}break;
}}}if(xmlhttp){xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==4){requestsQueue.shift();
if(requestsQueue.length>0){var nextUrl=requestsQueue[0];
xmlhttp.open("GET",nextUrl,true);
xmlhttp.send();
}}};
}}}w.onbeforeunload=function(e){if(prevOnBeforeUnload){prevOnBeforeUnload(e);
}send(1);
};
w.setInterval(function(){send();
},PUBLISH_PERIOD);
init();
replay();
CSM.ue.log=log;
